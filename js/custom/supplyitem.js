...
function buscaitem(){
	$.ajax({
		type: "POST",
		url: urlserver+"index.php/"+modulo+"/autocomplete2/",
		beforeSend: function(request){
			request.setRequestHeader('Token', sessionStorage.getItem("mysession"));
		},
		data:'keyword='+$('#itemss').val(),
		success: function(data){
			$("#suggesstion-box2").show();
			$("#suggesstion-box2").html(data);
			$("#itemss").css("background","#FFF");
		},
		error: function(XMLHttpRequest, textStatus, errorThrown) {
			$("#msgno").html( 'Error de conexión:' + textStatus +' '+ errorThrown);
			$(".alertbottom").show();
