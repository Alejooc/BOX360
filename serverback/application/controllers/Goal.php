...
class Goal extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('model_goal','model');		
	...
	public function get_form(){
		...
		if($this->input->post("id")>0){
			...
			$user['proccessn']=$this->model->get_proccessn($user['proccess']);
			$data['datos'] = $user;
		}
	...
	public function form_send($type){
		...
		$this->form_validation->set_rules('dategoal', 'Fecha ', 'required');
		$this->form_validation->set_rules('proccess', 'Proceso ', 'required');
				...
				if (!empty($registro)) {
					$this->model->update(
						array(
							...
							'dategoal' => $this->input->post('dategoal'),
							'proccess' => $this->input->post('proccess'),
	...
	function autocomplete($type=''){
		...
		if(empty($type)){
			$automodel = $this->model->get_proccess($var);
			$f="selectProccess";			
			echo '<ul id="proccess-list">';
		}
		foreach($automodel as $item) {
			echo '<li onClick='.$f.'('.$item->id.',"'.urlencode($item->name).'");>'.$item->area." - ".$item->name.'</li>';
		}
		echo '</ul>';
	}
}